...
LNP_TIMESERIES_FILE = "results/lnl_over_time_leave1.csv"
ZSCORE_FILE = "results/std_pace_vector.csv"
	...
	def __init__(self, lnp_list, threshold):
		...
		for i in range(len(lnp_list)):
			...
			if(aboveThreshold != current_state):
				...
				#Update the current_state and prevSegment
...
def saveEvents(timeSegments, zscore_timeseries, global_pace_timeseries, out_file):
	...
	for segment in timeSegments:
		#If the segment is below the threshold, it is an event
...
#Arguments:
	#lnp_timeseries - See readLnpTimeseries()
...
#Read the relative log-conditional probabiity (R(t)) timeseries from file.  Note that this file should be generated by likelihood_test_parallel.py
#Arguments:
	#filename - the file where the lnp timeseries is saved
#Returns:
	#A dictionary which contains the values of R(t), keyed by (date, hour, weekday)
...
if(__name__=="__main__"):
	...
	lnp_timeseries = readLnpTimeseries(LNP_TIMESERIES_FILE)
