...
def generateTimeSeriesLeave1(inDir):
	...
	(s_x, s_xxt, count) = computeGroupedStats(pace_timeseries)
	...
	for (date, hour, weekday) in sorted(pace_timeseries):
		...
		#Fit the full sample covariance
		#sig_test = estimate_cov_full(leave1_samp)
		...
		try:
			...
			print mu0
			print mu
			print matrix(mean(leave1_samp, axis=0))
			...
			print sig_full
			print estimate_cov_full(leave1_samp)
			exit()
